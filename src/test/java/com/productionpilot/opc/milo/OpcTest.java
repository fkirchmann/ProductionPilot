        ...
        for (var ref : browseResult.getReferences()) {
            ...
            if (ref.getNodeId().getIdentifier().toString().contains("Buerkle")
                ...
                if (ref.getNodeClass().equals(NodeClass.Variable)) {
                    ...
                    var nodeId =
                            ref.getNodeId().toNodeId(client.getNamespaceTable()).orElse(null);
                    log.debug("nodeId: {}", nodeId);

                    List<ReadValueId> readValueIds = List.of(
                    ...
                    NodeId dataType = (NodeId) client.read(0.0, TimestampsToReturn.Neither, readValueIds)
                            .get()
                            .getResults()[0]
                            .getValue()
                            .getValue();
