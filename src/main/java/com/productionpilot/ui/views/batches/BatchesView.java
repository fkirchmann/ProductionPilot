...
public class BatchesView extends VerticalLayout {
    ...
    private void init() {
        ...
        create.add("Create");
        create.getSubMenu().addItem(new LineAwesomeIcon("la la-folder"),
                            e -> batchDialog.openForCreation(this::select).setParentBatch(getSelectedBatch()))
                        .add(" Batch...");
        create.getSubMenu().addItem(new LineAwesomeIcon("la la-list-ul"),
        ...
        grid.addSelectionListener(event -> {
            ...
            if (selected != null && selected.isBatch()) {
                exportButton.getElement().setAttribute("onclick", "window.open('" +
                        BatchApi.getLinkToBatchExport(selected.batch) + "')");
    ...
    private void configureGrid() {
        ...
        grid.addComponentHierarchyColumn(b -> {
                    ...
                    layout.add(b.isBatch() ? new LineAwesomeIcon("la la-folder ")
                            : new LineAwesomeIcon("la la-list-ul"));
                    layout.add(new Text(b.getName()));
                    return layout;
                }).setHeader("Name")
                .setFlexGrow(1).setResizable(true).setSortable(true);
        grid.addColumn(BatchTreeData::getLastModified).setHeader("Modified")
                .setFlexGrow(0).setWidth("20ch").setResizable(true).setSortable(true);
        grid.addColumn(new ComponentRenderer<>(b -> {
                    var menuBar = new MenuBar();
                    menuBar.addThemeVariants(MenuBarVariant.LUMO_TERTIARY_INLINE);
                    menuBar.getStyle()
                            .set("margin-top", "-10px !important")
                            .set("margin-bottom", "-10px !important");
                    ...
                    if(b.isBatch()) {
                        ...
                        create.getSubMenu().addItem(new LineAwesomeIcon("la la-list-ul"),
                                        e -> {
                                            select(b.batch);
                                            batchMachineDialog.openForCreation(this::select).setBatch(b.batch);
                                        })
                                .add(" Measurement Range...");
                    } else {
                    ...
                    menuBar.addItem(new Icon("lumo", "edit"),
                            e -> {
                                if (b.isBatch()) {
                                    select(b.batch);
                ...
                .setFlexGrow(0).setWidth("12ch");
        refresh();
        add(grid);
    }

    private void refresh() {
        if(lazyUIRefresher.refreshIfNecessary(grid, batchService.findAll(), batchMachineService.findAll(),
                (grid, batches, batchMachines) -> {
                        var batchToChildBatches = batches.stream()
                                .filter(b -> b.getParentBatch() != null)
                                .collect(Collectors.groupingBy(Batch::getParentBatch,
                                        Collectors.toCollection(ArrayList::new)));
                        var batchToChildBatchMachines = batchMachines.stream()
                                .collect(Collectors.groupingBy(BatchMachine::getBatch,
                                        Collectors.toCollection(ArrayList::new)));
                        grid.setItems(
                            // Top level batches
                            batches.stream().filter(Batch::isTopLayer).map(BatchTreeData::new).toList(),
                            // For a batch, this returns its child batches and child batch machines
                            b -> b.isBatch() ?
                                    Stream.concat(batchToChildBatches.getOrDefault(b.batch, new ArrayList<>())
                                                    .stream().map(BatchTreeData::new),
                                                batchToChildBatchMachines.getOrDefault(b.batch, new ArrayList<>())
                                                        .stream().map(BatchTreeData::new)
                                        ).toList()
