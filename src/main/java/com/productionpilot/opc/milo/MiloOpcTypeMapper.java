...
public class MiloOpcTypeMapper {
    ...
    public static OpcStatusCode mapStatusCode(StatusCode code) {
        var nameAndDesc = StatusCodes.lookup(code.getValue())
    ...
    public static @Nullable OpcMeasuredValue mapMeasuredValue(@Nonnull OpcNode node, @Nonnull DataValue wrappedValue,
                                                              @Nonnull Instant clientTime) {
        ...
        if(value != null) {
            var sourceTime = Optional.ofNullable(wrappedValue.getSourceTime())
                    .map(DateTime::getJavaInstant).orElse(null);
