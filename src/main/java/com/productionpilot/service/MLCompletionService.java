...
public class MLCompletionService {
    ...
    public List<String> getCompletion(@Nonnull OpcNode node, @Nonnull String startsWith) {
        ...
        try {
            result = service.createCompletion(completionRequest).getChoices().stream()
                    .map(c -> startsWith + c.getText())
                    .distinct()
    ...
    private String getPromptPrefix() {
        var parameterNodes = parameterRecordingService.listSubscribedItems().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getNode()));
        var parametersNewestFirst = new ArrayList<>(parameterNodes.keySet().stream()
                .sorted(Comparator.comparing(Parameter::getId).reversed())
                .toList());
        ...
        for (Map.Entry<Machine, List<Parameter>> entry : parametersNewestFirst.stream()
                .collect(Collectors.groupingBy(Parameter::getMachine))
                .entrySet()) {
